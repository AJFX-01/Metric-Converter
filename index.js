...
// Nested JSON objects declaring all MAIN objects quantities
let quantities = [{
        // For Qunantity Object length
        name: 'Length',
        id: 'length',
        defUnitIndex: 1,
        // Nested Object (UNIT) inside the main object (LENGTH)  
        units: [
            ["kilometer", 1000],
            ["meter", 1],
            ["centimeter", 0.01],
    ...
    {   // Quantity Mass
        ...
        id: 'mass',
        defUnitIndex: 0,
        // Nested Object (UNIT) inside the main object (MASS)
        units: [
            ["kilogram", 1],
            ["gram", 1e-3],
            ["milligram", 1e-6],
    ...
    {   
        ...
        id: 'time',
        defUnitIndex: 7,
        // Nested Object (UNIT) inside the main object (TIME) with their equivalent units
        units: [
    ...
    {   
        ...
        // Nested Object (UNIT) inside the main object (TEMPERATURE) with their equivalent units
        units: [
    ...
    {   
        ...
        // Nested Object (UNIT) inside the main object (Angle) with their equivalent units
        units: [
    ...
    {   
        ...
        // Nested Object (UNIT) inside the main object (AREA) with their equivalent units
        units: [
    ...
    {   
        ...
        // Nested Object (UNIT) inside the main object (VOlume) with their equivalent units
        units: [
    ...
    {   
        ...
        // Nested Object (UNIT) inside the main object (Digital Storage) with their equivalent units
        units: [
            ["bits", 0.125],
            ["bytes", 1],
            ["kilobits", 128],
...
// FUnction to ge the quantity ID,
...
// Function to load the quantity by using the ID, when a button is selected
...
// to find the Unit by name, which returns the particular quantity unit
...
// Function to load the unit of each quauntity
function loadUnits() {
...
// Function to check if the number is numeric or not
function isNumber(n) {
    return !isNaN(parseFloat(n)) && isFinite(n);
}

// Function to convert each index units, taking four arguments
function convertInternal(topUnitIndex, value, bottomUnitIndex, inputResult) {
    // declaring a variable to topfactor to pick the first unit in the list of the arrays
    let topUnitFactor = quantity.units[topUnitIndex][1];
    // decalring a variable fo bottom unitfactor 
...
// Function to convert the unuits
function convert() {
    // decalre a variable value whcih takes in a parse float
    let value = parseFloat(inputTop.value);
    // Check if the value is a not number
    if (isNaN(value)) {
        inputBottom.value = ''; // shows an empty string
    } else {
        // if its a number, 
...
// Function to return back to original value, input on the column below
